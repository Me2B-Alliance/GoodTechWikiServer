import Ajv from 'ajv'
import { JSONSchemaBridge } from 'uniforms-bridge-json-schema'
import { LongTextField, SelectField } from 'uniforms-bootstrap4'

const engagements = [
  'Product Design & Dev Practices', 'Technology Standards',
  'Industry Advocacy and Education', 'Tools/Infrastructure/Services Development for B-s',
  'Tools Development for Me-s', 'Product Testing', 'Org Compliance and Testing',
  'Regulation, Legal, Policy', 'Consumer Advocacy and Education', 'Other'
]

const types = ['Organization', 'Event', 'Product', 'WorkingGroup', 'Publication']

const engagementsSchema = engagements.map((engagement) => ({
  label: engagement,
  value: engagement
}))

const typeSchema = types.map((type) => ({
  label: type,
  value: type
}))

const baseSchema = {
  title: 'Organization',
  type: 'object',
  properties: {
    uuid: {
      title: 'Uuid',
      type: 'string'
    },
    '@type': {
      label: 'Type',
      type: 'string',
      uniforms: { component: SelectField },
      options: typeSchema,
      allowedValues: types
    },
    name: {
      label: 'Organization Name *',
      type: 'string'
    },
    lisa: {
      label: 'Category *',
      type: 'string',
      uniforms: { component: SelectField },
      options: engagementsSchema,
      allowedValues: engagements
    },
    about: {
      label: 'Description',
      type: 'string',
      uniforms: { component: LongTextField }
    },
    dateFounded: {
      label: 'Date Founded',
      type: 'string'
    },
    dateEnded: {
      label: 'Date Ended',
      type: 'string'
    },
    organizationType: {
      label: 'Organization Type',
      type: 'string'
    },
    parentOrg: {
      label: 'Parent Org',
      type: 'string'
    },
    website: {
      label: 'Website',
      type: 'string'
    },
    partners: {
      label: 'Partners',
      type: 'string'
    },
    twitter: {
      label: 'Twitter Profile',
      type: 'string'
    },
    linkedin: {
      label: 'Linkedin Profile',
      type: 'string'
    },
    github: {
      label: 'Github Profile',
      type: 'string'
    },
    scope: {
      label: 'Scope',
      type: 'string'
    },
    sector: {
      label: 'Sector',
      type: 'string'
    },
    tags: {
      title: 'Tags',
      type: 'array',
      items: {
        0: String
      }
    },
    digitalHarmsAddressed: {
      label: 'Digital Harms Addressed',
      type: 'array',
      items: {
        0: String
      }
    },
    keyPeople: {
      label: 'Key People',
      type: 'array',
      items: {
        0: String
      }
    },
    activities: {
      title: 'Activities',
      type: 'array',
      items: {
        0: String
      }
    },
    locations: {
      title: 'Locations',
      type: 'array',
      items: {
        0: String
      }
    },
    purpose: {
      title: 'Purpose',
      type: 'array',
      items: {
        0: String
      }
    },
    me2bRelationship: {
      label: 'Me2b Relationship',
      type: 'string'
    },
    status: {
      label: 'Status',
      type: 'string'
    },
    techFocus: {
      label: 'Tech Focus',
      type: 'string'
    },
    relevantPublications: {
      label: 'Relevant Publications',
      type: 'string'
    },
    funding: {
      label: 'Funding',
      type: 'string'
    },
    annualBudget: {
      title: 'Annual Budget',
      type: 'string'
    },
    audience: {
      title: 'Audience',
      type: 'string'
    },
    productsAndOrServices: {
      title: 'Products And Or Services',
      type: 'string'
    },
    facebook: {
      title: 'Facebook',
      type: 'string'
    },
    wikipedia: {
      title: 'Wikipedia',
      type: 'string'
    }
  },
  required: ['name', 'lisa']
}

const ajv = new Ajv({ allErrors: true, useDefaults: true })

function createValidator(schema) {
  const validator = ajv.compile(schema)

  return (model) => {
    validator(model)
    return validator.errors?.length ? { details: validator.errors } : null
  }
}

const schemaValidator = createValidator(baseSchema)

export default new JSONSchemaBridge(baseSchema, schemaValidator)
