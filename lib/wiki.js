...
class Wiki {
  ...
  async putDdocs() {
    ...
    function techFocusMapper(doc) {
      // eslint-disable-next-line no-undef
      doc.techFocus && emit(doc.techFocus)
    }
    function organizationTypeMapper(doc) {
      // eslint-disable-next-line no-undef
      doc.organizationType && emit(doc.organizationType)
    }
    function sectorMapper(doc) {
      // eslint-disable-next-line no-undef
      doc.sector && emit(doc.sector)
    }
    ...
    const ddoc = {
      ...
      views: {
        ...
        techFocus: {
          map: techFocusMapper.toString(),
          reduce: reducer.toString()
        },
        organizationType: {
          map: organizationTypeMapper.toString(),
          reduce: reducer.toString()
        },
        sector: {
          map: sectorMapper.toString(),
          reduce: reducer.toString()
        }
      }
    }
